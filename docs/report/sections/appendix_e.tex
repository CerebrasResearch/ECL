...
\end{enumerate}
Expected: ECL stabilizes rapidly for models with concentrated influence (small ECD) and more slowly for models with diffuse influence.
...
\subsection{Sensitivity summary protocol}

We recommend reporting a sensitivity summary table for each model:

\begin{table}[H]
\centering
\caption{Proposed format: hyperparameter sensitivity summary.}
\label{tab:sensitivity}
\small
\begin{tabular}{@{}l l l@{}}
\toprule
\textbf{Hyperparameter} & \textbf{Range tested} & \textbf{$\widehat{\ECL}_{0.9}$ range (bp)} \\
\midrule
$n$ (MC samples) & 10--500 & --- \\
$b$ (block size) & 1--512 bp & --- \\
$B$ (bootstrap) & 100--5000 & --- \\
Layer $l$ & 1--$L$ & --- \\
Metric $d_\cZ$ & Eucl./Cos./Mahal. & --- \\
Perturbation type & Sub/Shuf/Markov/Gen & --- \\
\bottomrule
\end{tabular}
\end{table}
