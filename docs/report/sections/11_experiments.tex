...
\paragraph{Scope of the reported numbers.}
The figures and tables in this section demonstrate the \emph{statistical workflow} of the framework on a controllable synthetic substrate; deploying the framework on real model weights is the empirical follow-up of this work and not the contribution of this manuscript. Specifically, all numerical values reported in \cref{tab:ecl_estimates,tab:utilization,tab:perturbation_sensitivity,tab:multiscale_block,tab:pairwise_comparison,tab:biological_validation} should be read as the output of the framework applied to surrogate decay profiles calibrated to each architecture's expected behavior, not as measurements obtained from the model weights themselves. We retain a single internally consistent scale across all experimental tables so that the cross-model comparisons exemplify the workflow without committing to specific empirical claims about the named models.
...
and \textbf{Nucleotide Transformer v3-650M} (ESM/BERT, $\sim$12~kb). The full taxonomy is given by \cref{tab:model_taxonomy} in \cref{sec:related}.
...
\caption{Log-scale influence profiles $\widehat{\cI}(d;r)$ vs.\ distance $d$ (bp) for eight models (synthetic surrogates), averaged over promoter-centered loci. Two perturbation types shown: random substitution (blue) and dinucleotide shuffle (orange). Shaded bands show $\pm$1~SE. Models with longer decay lengths show slower influence falloff.}
...
\input{tables/tab02_ecl_estimates.tex}

\input{tables/tab03_utilization.tex}
...
\input{tables/tab04_perturbation_sensitivity.tex}

\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig05_perturbation_scatter.pdf}
\caption{Scatter plot of ECL$_{0.9}$(shuffle) vs.\ ECL$_{0.9}$(substitution) across 500 promoter loci for eight models (synthetic surrogates), colored by model. Diamonds mark model means. The positive correlation confirms perturbation robustness; the systematic offset below the diagonal reflects the broader perturbation window of dinucleotide shuffle.}
...
\input{tables/tab05_multiscale_block.tex}
...
\input{tables/tab06_pairwise_comparison.tex}

\subsection{Experiment 9: Biological validation}

For known long-range enhancer--gene pairs, we check whether model ECL reaches the known regulatory distance. The two best-characterized long-range pairs in our list are SHH/ZRS~\citep{Karollus2023Limited} and HBB/LCR; the remaining locus pairs in \cref{tab:biological_validation} are reported with their commonly cited regulatory distances and serve as illustrative targets for the workflow rather than fully validated reference values, and a real-data deployment of this experiment should attach explicit citations or experimental measurements to each row.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig10_biological_validation.pdf}
\caption{Normalized influence profiles at three known long-range regulatory loci (SHH/ZRS, MYC/super-enhancer, SOX9/regulatory desert) for synthetic surrogates parameterized to each model's decay behavior. Red dashed lines mark known enhancer positions. Only the long-context surrogate retains appreciable influence at the enhancer distance.}
\label{fig:biological}
\end{figure}

\input{tables/tab07_biological_validation.tex}
...
\caption{gNIAH sensitivity vs.\ insertion distance for three regulatory motifs across surrogates with different context lengths. Longer-context surrogates maintain sensitivity at greater distances. CTCF motifs are detected at longer range than SP1, consistent with CTCF's role as a long-range chromatin organizer.}
\label{fig:gniah}
\end{figure}
