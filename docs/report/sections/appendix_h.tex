...
Bidirectional Mamba (BiMamba) with reverse-complement equivariance. Achieves competitive performance with 10$\times$ fewer parameters than transformer baselines on variant effect prediction. The bidirectional architecture is significant for ECL: both upstream and downstream context can influence each position, unlike autoregressive models.

\paragraph{\citet{Brixi2025Evo2} --- Evo~2.}
40B-parameter StripedHyena-Transformer hybrid processing 1~Mb inputs, trained on 9.3~trillion base pairs across all domains of life. Currently the largest genomic language model. The hybrid architecture (predominantly SSM with sparse attention) creates a distinctive ECL profile that may differ from pure transformers or pure SSMs.
