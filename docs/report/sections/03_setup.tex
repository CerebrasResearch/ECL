...
Let $\cV$ be a finite alphabet (e.g., $\{A,C,G,T\}$ for DNA) and define the raw input space $\cX_{\mathrm{raw}} := \cV^{L_{\mathrm{bp}}}$, where $L_{\mathrm{bp}}$ is the number of base pairs. Models that use tokenization (BPE, $k$-mers) operate on a token space $\cX := \cV_{\mathrm{tok}}^{L}$ with $L \leq L_{\mathrm{bp}}$. We define a \emph{position mapping} $\phi:\{1,\dots,L\}\to\{1,\dots,L_{\mathrm{bp}}\}$ that maps each token index to its starting base-pair coordinate. We assume $\phi$ is non-decreasing and that the preimage $\phi^{-1}(\{k\})$ of each base-pair index is a contiguous (possibly empty) block of token indices, so that token-space and bp-space windows are mutually convertible without ambiguity. For single-nucleotide models (HyenaDNA, Evo), $\phi$ is the identity.
...
\begin{definition}[Idealized distribution-aware perturbation kernel]\label{def:perturbation}
A family $\{\Pi_S\}$ is \emph{distribution-aware} for $P_X$ in the idealized sense if, for each $S$, coordinates outside $S$ are preserved and coordinates in $S$ are resampled according to the true conditional model:
\[
X^{(S)} \sim \Pi_S(\cdot\mid X) \quad \text{such that}\quad X^{(S)}_{-S}=X_{-S},\quad X^{(S)}_{S}\sim P_{X_S\mid X_{-S}}(\cdot\mid X_{-S}).
\]
\end{definition}

\begin{remark}[Practical surrogates and bias relative to the ideal]\label{rem:perturbation_surrogates}
Exact conditional resampling under $P_{X_S\mid X_{-S}}$ is rarely available in practice. The four kernels enumerated below are practical surrogates: random substitution and dinucleotide shuffle target the marginal or low-order conditional distribution rather than the true conditional, and so introduce a positive bias in influence energy by perturbing toward off-distribution sequences; $k$-th order Markov resampling reduces this bias by matching local sequence statistics; generative infilling from a pretrained masked language model approximates the true conditional and is therefore the closest to the ideal, at the cost of additional model evaluations. We recommend reporting ECL under multiple surrogates and treating consensus across kernels as a robustness check.
\end{remark}
...
$\ECL_\beta(r)$ & Effective context length at level $\beta$, per locus $r$ (also written $\ECL_\beta(f,r)$) \\
$\ECL_\beta(f)$ & Locus-aggregate ECL: $\E_R[\ECL_\beta(f,R)]$ for a distribution of reference loci $R$ \\
...
$\ECL_\beta^\pm(r)$ & Directional ECL (upstream/downstream) at level $\beta$ \\
$\cI_{\mathrm{int}}(i,j;r)$ & Pairwise interaction influence (\cref{eq:interaction}) \\
$\ECLint_\beta(r)$ & Interaction ECL at level $\beta$ (\cref{def:ecl_int}) \\
$\CDS$ & Context Decay Spectroscopy mixture (\cref{def:cds}) \\
\bottomrule
\end{tabular}
\end{table}
