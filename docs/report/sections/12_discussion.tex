...
Comparing model ECL distributions to the known distribution of regulatory distances provides a principled assessment of whether a model captures biologically relevant long-range dependencies. Estimates of enhancer--promoter distance distributions vary across studies and assays (Hi-C, PCHi-C, CRISPR-based perturbation screens); widely cited surveys place the bulk of enhancer--promoter interactions within roughly $100$~kb of the target gene, with a long tail extending to $\sim$$1$~Mb~\citep{Karollus2023Limited}. Under such estimates, a model with ECL well below this typical interaction radius will systematically miss long-range regulatory dependencies; conversely, achieving ECL on the order of the interaction radius should be considered a minimum competency for regulatory genomics. We deliberately do not commit to a single percentile here, since the precise distribution depends on the underlying assay and dataset.

\subsection{Limitations}

\paragraph{Perturbation dependence.}
ECL is not an intrinsic property of $f$ alone: it depends on $\Pi$ and $P_X$. We recommend reporting ECL under multiple perturbation types and defining a \emph{consensus ECL} as the median across perturbation types. Reports should also document the provenance of $P_X$ (reference genome, sequence cohort, sampling protocol), since switching $P_X$ implicitly redefines the influence energies.
