...
We use the standard percentile bootstrap~\citep{EfronTibshirani1993Bootstrap}, applied at the level of sequences (the outer Monte Carlo unit), to produce confidence intervals for $\widehat{\ECL}_\beta(r)$.
...
\For{$b\leftarrow 1$ \KwTo $B$}{
  Resample sequence indices $t$ with replacement from $\{1,\dots,n\}$, keeping the inner perturbation samples for each resampled $t$ as a unit (block bootstrap over sequences)\;
...
\paragraph{Block-distance convention.}
The minimum-index convention $d(B_k) := \min_{i\in B_k}|i-r|$ is conservative: it associates each block with its nearest position to $r$, yielding the smallest plausible ECL upper bound. Alternative conventions (block midpoint, block maximum) shift estimates by at most $b$ bp and become asymptotically equivalent for $b \ll \ECL_\beta(r)$. We recommend the minimum convention by default and reporting the convention used.
...
Compute $\cI_{\le \ell}^{\mathrm{surr}}(r) = \sum_{d=0}^\ell n_d\,\hat{h}(d)$ analytically, where $n_d := |\{i:|i-r|=d\}|$ is the position multiplicity at distance $d$ ($n_0 = 1$ at the interior reference, $n_d \le 2$ for $d \ge 1$, with boundary corrections near sequence edges)\;
